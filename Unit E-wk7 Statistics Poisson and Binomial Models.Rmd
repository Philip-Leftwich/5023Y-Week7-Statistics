...
It would appear we **can** drop the interaction term *without* significantly affecting the fit of our model.


<details><summary>Q. If we have a model withoyt interactions in it, which Sum of Squares should we use to report? Type 1, II or III?</summary>
...
car::Anova(main_effect_count_model, type="II") ### car::Anova can do Type II or Type III
      
```

```{r, echo=FALSE}
car::Anova(main_effect_count_model, type="II") %>% 
  kableExtra::kbl() %>% 
  kableExtra::kable_minimal() 
```
...
I found that there was no significant interaction effect of Mass and Species ($\chi^2$ ~1,54 = 11.76, *P* = 0.22), suggesting that the relationship between increasing mass and the rate of begging is broadly the same across both species. 
...
```{r}
exp(coef(main_effect_count_model)[1]) ### Intercept - Incidence rate at x=0

exp(coef(main_effect_count_model)[2]) ### Change in the incidence rate with Mass

exp(coef(main_effect_count_model)[3]) ### Change in the incidence rate intercept by Species
...
```{r, eval=FALSE}
broom::tidy(main_effect_count_model, 
            exponentiate=T, 
            conf.int=T)
...
```{r, eval=FALSE}
broom::glance(main_effect_count_model)
```
...
```{r}
broom::augment(main_effect_count_model, cuckoo, type.predict="response") %>% 
  ggplot(aes(x=Mass,
             y=Beg,
             colour=Species))+
...
<details><summary> Mass 1.06 (95%CI: 1.05-1.07) </summary>

For every gram of bodyweight gained by a chick there was an average increase of 1.06 in the rate of begging calls per six seconds. 
...
<details><summary> SpeciesWarbler 0.96 (95%CI: 0.67-1.38) </summary>

Warblers had produce an average of 0.96 more calls per six seconds than Cuckoos, however this was not statistically significant in our analysis $\chi^2$_1_54 = 11.76, P = 0.22


</details>


Include these with the Chi statistics generated from your ANOVA tables - and you have a comprehensive results write-up. 

> *Note*
>
> Anova tables for poisson and binomial models use Chi not F because they are odds models not least squares models
